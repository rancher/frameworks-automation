#!/usr/bin/env bash
# bump-webhook.sh — bumps webhook in a locally checked out rancher/rancher
# repo. Adapted from rancher/webhook's
# .github/workflows/scripts/release-against-rancher.sh, with these changes
# to fit the release-automation bumper's contract:
#
#  1. Single positional argument (new version). The chart-prefixed full
#     version is looked up at runtime from rancher/charts' index.yaml at
#     the ref named by the CHART_REF env var (any git ref — branch, tag,
#     or SHA). The chart bump must have merged into rancher/charts before
#     this script runs (cascade enforces that via the `order` edge).
#  2. No git operations. The bumper does one final `git add -A && git commit`
#     after every script in the bundle has run.
#  3. When the previous and new full versions match, exit 0 silently. The
#     bumper uses `git status` to detect the no-op case and skips the PR.
#
# Usage: bump-webhook.sh <new-version>
#   e.g. CHART_REF=dev-v2.15 bump-webhook.sh v0.5.0-rc.14
#
# Required tools in PATH: yq (v4), curl, go (for `go generate`).
set -euo pipefail

if [[ $# -ne 1 ]]; then
    echo "usage: $0 <new-webhook-version>" >&2
    exit 2
fi
NEW_VERSION="$1"
CHART_REF="${CHART_REF:-}"

if ! echo "$NEW_VERSION" | grep -qE '^v[0-9]+\.[0-9]+\.[0-9]+(-rc\.[0-9]+)?$'; then
    echo "Error: version $NEW_VERSION must be vX.Y.Z or vX.Y.Z-rc.N" >&2
    exit 1
fi
if [[ -z "$CHART_REF" ]]; then
    echo "Error: CHART_REF env var must be set to a rancher/charts ref — branch, tag, or SHA (e.g. dev-v2.15)" >&2
    exit 1
fi

NEW_SHORT="${NEW_VERSION#v}"

# Resolve `<chart>+up<dep>` from rancher/charts' index.yaml. rancher/charts is
# public, so no auth is needed for raw.githubusercontent.com. The bare-ref
# form (no `refs/heads/`) accepts branches, tags, or SHAs — production passes
# a branch name; integration tests can pin a SHA for reproducibility.
INDEX_URL="https://raw.githubusercontent.com/rancher/charts/${CHART_REF}/index.yaml"
NEW_FULL=$(curl -fsSL "$INDEX_URL" \
    | yq -r ".entries.\"rancher-webhook\"[] | select(.appVersion == \"${NEW_SHORT}\") | .version" \
    | head -n1)
if [[ -z "$NEW_FULL" ]]; then
    echo "Error: no rancher-webhook entry with appVersion=${NEW_SHORT} in ${INDEX_URL} — wait for the chart bump to merge first" >&2
    exit 1
fi

if [[ ! -f ./build.yaml ]]; then
    echo "Error: ./build.yaml not found — run from a rancher repo root" >&2
    exit 1
fi

PREV_FULL=$(yq -r '.webhookVersion' ./build.yaml)
if [[ -z "$PREV_FULL" || "$PREV_FULL" == "null" ]]; then
    echo "Error: .webhookVersion missing from build.yaml" >&2
    cat ./build.yaml >&2
    exit 1
fi
if [[ "$PREV_FULL" == "$NEW_FULL" ]]; then
    echo "bump-webhook: already at $NEW_FULL; nothing to do"
    exit 0
fi

yq --inplace ".webhookVersion = \"${NEW_FULL}\"" ./build.yaml

go generate ./...

echo "bump-webhook: bumped to ${NEW_FULL}"
